...
async def get_switch_health(host: str, preferred_vendor: str = "auto") -> Tuple[Optional[Dict], Optional[Dict], Optional[str]]:
    ...
    try:
        ...
        print(f"[AUTH ERROR] {host}: {str(e)}")
        return None, None, f"Authentication failed: Check credentials for {host}"
    except NetmikoTimeoutException as e:
        print(f"[TIMEOUT ERROR] {host}: {str(e)}")
        return None, None, f"Connection timeout: {host} not reachable (check firewall/network)"
    except Exception as e:
        error_msg = str(e)[:150]
        print(f"[ERROR] {host}: {error_msg}")
